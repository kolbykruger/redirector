<template>
    <div>
        <PageHeading
            ...
            summary="We'll try and some matches to save you time."
        ></PageHeading>
        ...
        <section class="step step-1" v-if="step === 1">
            <div class="container">
                <h2>Exact match</h2>
                <p>
                    Would you like us to try and find some
                    <strong>exact</strong> matches?
                </p>
                <div class="buttons">
                    <button
            ...
            <div v-if="step >= 2">
                <section class="step step-2" v-if="step >= 2">
                    <div class="container" v-if="exactMatches.length >= 1">
                        <h2>
                            We found {{ exactMatches.length }} exact matches!
                        </h2>
                        <p>
                            We'll automatically remove these for you, since they
                            don't require redirects!
                        </p>
                        <div
                            class="stage-matches"
                            v-if="exactMatches.length >= 1"
                        >
                    ...
                    <div class="container">
                        ...
                        <div class="ignore-opts">
                            ...
                            <button
                                ...
                                :for="item"
                                v-for="(item, i) in ignoreList.ignorelist"
                                :key="i"
                                @click="removeFiletypeFromIgnoreList(item)"
                                title="Remove item"
                            >
                                {{ item }}
                            </button>
                            <div class="ignore-additions">
                                <input
                                    type="text"
                                    placeholder="Add .filetype to ignore-list..."
                                    v-model="ignoreListInput"
                                />
                                <button
                                    class="button"
                                    @click="addFiletypeToIgnoreList"
                                >
                                    Add
                                </button>
                            </div>
            ...
            <section class="step step-4" v-if="step >= 3">
                <div class="container">
                    <div v-if="partialMatches.length >= 1">
                        <h2>
                            We found {{ partialMatches.length }} partial
                            matches!
                        </h2>
                        <p>
                            Each redirect shown has one or more partial matches.
                            Please select the correct match (if any) and add it.
                        </p>
                        <div
                            class="stage-matches"
                            v-if="partialMatches.length >= 1"
                        >
                            <div
                                v-for="link in partialMatches"
                                :key="link.index"
                            >
                                <PartialMatchLink
                                    :redirectData="link"
                                    @childToParent="confirmRedirect"
                                ></PartialMatchLink>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        findPartialMatches() {
            ...
            arr1 = [
                ...new Map(
                    arr1.map(item => [item.link.pathname, item])
                ).values()
            ]
            arr2 = [
                ...new Map(
                    arr2.map(item => [item.link.pathname, item])
                ).values()
            ]
            ...
            let joinMatches = [].concat(
                probablyExactMatches,
                singleMatches,
                multiMatches
            )
